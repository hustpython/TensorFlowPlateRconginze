...
if __name__ =='__main__' and sys.argv[1]=='train':
    ...
    for i in range(0+10,NUM_CLASSES+10):
        dir = '/home/mxq/Codes/TensorflowPlateRecognize/data/tf_car_license_dataset/train_images/training-set/letters/%s/' % i           # 这里可以改成你自己的图片目录，i为分类标签
    ...
    val_count = 0
    for i in range(0+10,NUM_CLASSES+10):
        dir = '/home/mxq/Codes/TensorflowPlateRecognize/data/tf_car_license_dataset/train_images/validation-set/letters/%s/' % i           # 这里可以改成你自己的图片目录，i为分类标签
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                val_count += 1
...
if __name__ =='__main__' and sys.argv[1]=='predict':
    ...
    with tf.Session() as sess:
        ...
        for n in range(1,9):
            path = "/home/mxq/Codes/TensorflowPlateRecognize/data/tf_car_license_dataset/test_images/letters/%s.bmp" % (n)
            img = Image.open(path)
            width = img.size[0]
            height = img.size[1]
